      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        ...
        <View className="bg-gray-800 rounded-2xl p-2 mb-2 shadow">
          <Text className="text-lg text-white font-semibold mb-2">
            ✅ Richtig beantwortet: { answeredCorrectly.length }
          </Text>
          {answeredCorrectly.map((q, i) => (
            <Text key={i} className="text-gray-300 ml-1 bg-green-900 p-1"
                style={{
                  borderBottomLeftRadius: i === answeredCorrectly.length - 1 || answeredCorrectly.length == 1 ? 10 : 0,
                  borderBottomRightRadius: i === answeredCorrectly.length - 1 || answeredCorrectly.length == 1 ? 10 : 0,
        ...
        <View className="bg-gray-800 rounded-2xl p-2 mb-2 shadow">
          <Text className="text-lg text-white font-semibold mb-2">
            ❌ Falsch beantwortet: {answeredWrong.length}
          </Text>
          {answeredWrong.map((q, i) => (
            <Text key={i} className="text-gray-300 ml-1 bg-red-900 p-1"
                style={{
                  borderBottomLeftRadius: i === answeredWrong.length - 1 ? 10 : 0,
                  borderBottomRightRadius: i === answeredWrong.length - 1 ? 10 : 0,
                    borderTopLeftRadius: i === 0 ? 10 : 0,
                    borderTopRightRadius:  i === 0 ? 10 : 0,
          ...
          <CustomButton
            title="Nochmal mit neuen Fragen"
            handlePress={() => {
            ...
            />
            <CustomButton 
            title="Nochmal mit gleichen Fragen"
            handlePress={() => {
