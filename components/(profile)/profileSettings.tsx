...
const ProfileSettings = () => {
  ...
  const { t } = useTranslation();
    const {user, language,setNewLanguage, userUsage, setUserUsage } = useGlobalContext() 
      const [ selectedLanguage, setSelectedLanguage ] = useState<string | null>("DEUTSCH")
      useEffect(() => {
        if(language) {
    ...
    const [ userDataKathegory, setUserDataKathegory] = useState<userDataKathegory>()
    const [ loading, setLoading] = useState(true)
    const [selectedColorMode, setSelectedColorMode] = useState<string | null>("Darstellung");
    ...
    useEffect(() => {
      ...
      async function fetchUserData() {
        ...
        setSelectedColorMode(userData && userData.darkmode ? t("profileSettings.darkmode") : t("profileSettings.darkmode"))

        setUserData(userData)
        const userDataKathegoryDoc = await loadUserDataKathegory(user.$id);
        const userDataKathegoryTyped: userDataKathegory | null = userDataKathegoryDoc as unknown as userDataKathegory;
        setSelectedLanguage(
      ...
      <Modal
        ...
        >
          <View className='flex-1 items-center justify-center p-2' >
            ...
            >
              <Text className='text-white font-bold text-[15px] my-3'>{t("profileSettings.actioncodeText")}</Text>
              <TextInput
              ...
              <View className='flex-1 flex-row items-center justify-center'>
                <CustomButton title={t("profileSettings.cancel")} handlePress={() => setModalVisible(false)} containerStyles={"w-[50%] bg-gray-800 mx-1 border-w-[1px] border-gray-500"}/>
                <CustomButton title={t("profileSettings.ok")} handlePress={toggleModal} containerStyles={!isFocused && firstFocus && text == "" ? "w-[50%] bg-gray-700 mx-1 border-gray-700" :"w-[50%] bg-blue-500 mx-1"} textStyles={"text-gray-300"} disabled={!isFocused && firstFocus && text == ""}/>
              </View>
            </View>
          </View>
        </Modal>
                  ...
                  <TouchableOpacity className='flex-1 justify-start pt-5 items-center' onPress={()=> {setIsError(false); setIsSccess(false)} }
                    >
                      ...
                      >
                          ...
                          isSuccess  ? t("profileSettings.successActionCode") 
  ...
  return (
    <View className='flex-1 items-center '>
          ...
          content={()=> {
                return(
                    <View className='flex-1 items-center '>
                        ...
                        {personalInput(user.name,t("profileSettings.vorname"), (text) => updateUserName(text))}
                        {personalInput(user.email,t("profileSettings.email"), (text) => updateUserEmail(text),true) }
                        {
                            !user.emailVerification ?
                            <View className={`${isVertical ? "flex-row w-[96%] justify-between items-center" : "justify-start items-start"} py-2 `}>
                                <Text className='text-red-300 font-bold text-[12px]'>{t("profileSettings.emailvalid")}</Text>
                                <TouchableOpacity className='py-2 px-3 m-2 rounded-full border-gray-500 border-[1px]'
                                 onPress={
                                  ()=> {validateEmail()}
                                 }
                                >
                                  <Text className='text-gray-300 font-bold text-[12px]'>{t("profileSettings.emailrenew")}</Text>
                                </TouchableOpacity>
                            </View>
                            ...
                            <Text>
                                <Text className='text-green-300 font-bold text-[12px]'>{t("profileSettings.emailvalid2")}</Text>
                            </Text>
                            </View>
                        }
                        ...
                        <View className='justify-start w-full'>
                          {
                            ...
                            <View className='w-full'>
                              ...
                              >{t("profileSettings.schoolgrade")}</Text>
                        ...
                        <TouchableOpacity 
                          ...
                          >
                          <Text className='text-blue-500 font-bold'>{t("profileSettings.editEducationGoals")}</Text>
                          </TouchableOpacity>
                        </View>
                        </View>
      ...
      <InfoModule 
        ...
        content={()=> { return(
              <View className={`flex-1  ${isVertical ? "flex-row justify-start  items-center" : "items-start"}`}>
                <OptionSelector title={t("profileSettings.colorMode")} options={colorOptions} selectedValue={selectedColorMode} setSelectedValue={setSelectedColorMode} onChangeItem={updateColorMode}/>
                <OptionSelector title={t("profileSettings.language")} options={languageoptions} selectedValue={selectedLanguage} setSelectedValue={setSelectedLanguage} onChangeItem={updateLanguage}/>
              </View>
      ...
      content={()=> { return(
        <View>
          <SettingsOption title={t("profileSettings.help") } iconName={"life-ring"} handlePress={()=> router.push("/contact") }/> 
          <SettingsOption title={t("profileSettings.policys") } iconName={"shield-alt"} handlePress={()=> router.push("/policys") }/>
          <SettingsOption title={t("profileSettings.actioncode") } iconName={"bolt"} item={modal()} handlePress={()=> setModalVisible(true)}/>  
          <SettingsOption title={t("profileSettings.logout") } iconName={"sign-out"} handlePress={ ()=>    router.push("/sign-out")}/>
          <SettingsOption title={t("profileSettings.deleteAccount") } iconName="trash" bottom={true} handlePress={ ()=>    router.push("/delete-account")}/>
