...
const SearchList = ({
  ...
  return (
    <View
      className="flex-1 w-full bg-gray-900 max-w-[600px] max-h-[700px] rounded-[10px] pb-2 "
          ...
          renderItem={({ item, index }) => (
            <TouchableOpacity
              ...
              className={`flex-row p-2 border-gray-800 border-[1px] rounded-[10px] ${
                isSelected(item) ? "bg-blue-700 " : "bg-gray-800"
              }  items-center justify-start m-2`}
            >
              <Icon name={item.icon} size={20} color="white" />
              <Text
                className="text-gray-100 font-semibold text-[15px] text-start ml-2"
