import { Client,Account,ID, Avatars, Databases, Storage,Functions  } from 'react-native-appwrite';
import { Linking } from "react-native";
import { getSessionFromMMKV, setSessionInMMKV } from './mmkvFunctions';
import { checkMMKVTimestampExpiry, setMMKVSessionTimestamp } from './mmkvUpdateTimestamps';
...
export const checkSession = async () => {
    try {
        if (checkMMKVTimestampExpiry()) {
            const session = await account.get(); 
            setSessionInMMKV(session)
            setMMKVSessionTimestamp();
            return session;
        } else {
            return getSessionFromMMKV(); 
        }
    } catch (error) {
