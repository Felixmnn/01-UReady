    ...
    <View>
      <SubHeader title={t("createModule.materials")} />
              <View className="w-full flex-row flex-wrap justify-start items-center my-2 ">
                {selectedSession && items.filter(item => item.sessionID == selectedSession.id).length > 0 ? (
                  items.filter(item => item.sessionID == selectedSession.id).map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        <Icon name={iconMap[item.type as "TOPIC" | "PEN" | "FILE"]} size={15} color="#4B5563" />
                  ...
                  <TouchableOpacity
                    className="bg-[#0c111d] flex-row p-2 mt-4  border-gray-800 border-[1px] rounded-[10px] items-center justify-center shadow-lg"
                  >
