...
const Header = ({setSelectedScreen,selectedModule, selected, sessions, setSessions, addDocument, setQuestions, questions}) => {
  ...
  return (
    <View className={`${!isVertical ? "bg-[#0c111d]" : null}`}>
        <ModalNewQuestion addDocument={addDocument} sessions={sessions} selected={selected} module={selectedModule} isVisible={isVisible} setIsVisible={setIsVisible} setSelected={setSelectedScreen} selectAi={()=> {setIsVisible(false);setIsVisibleAI(true) } } /> 
        <AiQuestion questions={questions} setQuestions={setQuestions} selectedSession={sessions[selected]} isVisible={isVisibleAI} setIsVisible={setIsVisibleAI} selectedModule={selectedModule} />
        ...
        <View className='flex-row w-full justify-between p-4 items-center'>
                ...
                <View className='w-full  flex-row px-4 justify-between items-center'>
                    <Text className='text-gray-200 font-bold text-2xl'>{sessions.length < sessions ?  sessions[selected].title : "Alle Fragen"}</Text>
