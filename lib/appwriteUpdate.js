...
import { addUnsavedModuleToMMKV, updateModuleInMMKV, updateModuleQuestionListInMMKV } from './mmkvFunctions';
...
export async function updateModuleData (id, data) {
    try {
        ...
        console.log("✅ Module Data updated successfully:", res);
        return res;
    } catch (error) {
        if (__DEV__) {
...
export async function updateUserUsageData (data) {
    try {
        ...
        if (__DEV__) {
        console.error("❌Error while updating User Data uodateUserUsageData()", error.message);
        }
    }
